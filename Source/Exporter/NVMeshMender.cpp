...
namespace selene
{
        ...
        public:
                                       ...
                                       const float& minCreaseAngle)
                {
                        ...
                        Vector3d tmp1 = t1->normal;
                        Vector3d tmp2 = t2->normal;
                        tmp1.normalize();
                        tmp2.normalize();

                        if(tmp1.dot(tmp2) >= minCreaseAngle)
                        {
                                return true;
                        }
                        else if((tmp1 == Vector3d()) && (tmp2 == Vector3d()))
                        {
        ...
        public:
                                       ...
                                       const float& minCreaseAngle)
                {
                        ...
                        // for checking the angle, we want these to be normalized,
                        // they may not be for whatever reason
                        Vector3d tmp1 = t1->tangent;
                        Vector3d tmp2 = t2->tangent;
                        tmp1.normalize();
                        tmp2.normalize();
        ...
        public:
                                       ...
                                       const float& minCreaseAngle)
                {
                        ...
                        // for checking the angle, we want these to be normalized,
                        // they may not be for whatever reason
                        Vector3d tmp1 = t1->binormal;
                        Vector3d tmp2 = t2->binormal;
                        tmp1.normalize();
                        tmp2.normalize();
        ...
        bool operator <(const Vector3d& lhs, const Vector3d& rhs)
        {
                ...
                if(std::fabs(lhs.x - rhs.x) < SELENE_EPSILON)
                {
                        if(std::fabs(lhs.y - rhs.y) < SELENE_EPSILON)
                        {
                                if(std::fabs(lhs.z - rhs.z) < SELENE_EPSILON)
                                        return false;
                                else
                                        return (lhs.z < rhs.z);
                        }
        ...
        void MeshMender::ProcessNormals(TriangleList& possibleNeighbors,
                                        ...
                                        Vector3d workingPosition)
        {
                // a fresh group for each pass
                NeighborGroupList neighborGroups;
                ...
                std::vector<Vector3d> groupNormalVectors;
                ...
                for(i = 0; i < neighborGroups.size(); ++i)
                {
                        ...
                        for(auto it = thisGroupIndices.begin(); it!= thisGroupIndices.end(); ++it)
                                         ...
                                         Vector3d workingPosition)
        {
                ...
                std::vector<Vector3d> groupTangentVectors;

                for(i = 0; i < neighborGroups.size(); ++i)
                {
                        Vector3d gtang;

                        for(unsigned int t = 0; t < neighborGroups[i].size(); ++t)
                        {
                                          ...
                                          Vector3d workingPosition)
        {
                ...
                std::vector<Vector3d> groupBinormalVectors;
        ...
        bool MeshMender::Mend(std::vector<Vertex>& theVerts,
                              ...
                              const CylindricalFixOption fixCylindricalWrapping)
        {
                ...
                for(auto vert = m_VertexChildrenMap.begin();
                    vert != m_VertexChildrenMap.end();
                    ++vert)
                {
                        Vector3d workingPosition = vert->first;
        ...
        void MeshMender::BuildGroups(Triangle* tri,
        ...
        bool MeshMender::TriHasEdge(const Vector3d& p0,
                                    const Vector3d& p1,
                                    const Vector3d& triA,
                                    const Vector3d& triB,
                                    const Vector3d& triC)
                                    ...
                                    std::vector<Vertex>& theVerts)
        {
                ...
                if(m_RespectExistingSplits == RESPECT_SPLITS)
                        return SharesEdgeRespectSplits(triA, triB);

                Vector3d a1 = theVerts[triA->indices[0]].pos;
                Vector3d b1 = theVerts[triA->indices[1]].pos;
                Vector3d c1 = theVerts[triA->indices[2]].pos;

                Vector3d a2 = theVerts[triB->indices[0]].pos;
                Vector3d b2 = theVerts[triB->indices[1]].pos;
                Vector3d c2 = theVerts[triB->indices[2]].pos;
        ...
        void MeshMender::SetUpData(std::vector<Vertex>& theVerts,
                                   std::vector<unsigned int>& theIndices,
                                   std::vector<unsigned int>& mappingNewToOldVert,
                                   const NormalCalcOption computeNormals)
        {
                ...
                for(i = 0; i < theIndices.size(); i += 3)
                {
                        ...
                        // set id, to my index into m_Triangles
                        t.myID = m_Triangles.size();
                ...
                for(i = 0; i < m_Triangles.size(); ++i)
                {
                        for(size_t indx = 0; indx < 3; ++indx)
                        {
                                Vector3d v = theVerts[m_Triangles[i].indices[indx]].pos;
                                auto iter = m_VertexChildrenMap.find(v);
                                          ...
                                          const NormalCalcOption computeNormals)
        {
                if(computeNormals == CALCULATE_NORMALS)
                {
                        ...
                        {
                                Vector3d normalizedNorm;
                                normalizedNorm = t.normal;
                                normalizedNorm.normalize();

                                Vector3d finalNorm = (normalizedNorm * (1.0f - WeightNormalsByArea)) + (t.normal * WeightNormalsByArea);
                                t.normal = finalNorm;
                        }
                }
        ...
        void MeshMender::OrthogonalizeTangentsAndBinormals(std::vector<Vertex>& theVerts)
        {
                ...
                for(size_t i = 0; i < len; ++i)
                {
                        ...
                        Vector3d tmpTan  = theVerts[i].tangent;
                        Vector3d tmpNorm = theVerts[i].normal;
                        Vector3d tmpBin  = theVerts[i].binormal;

                        Vector3d newT = tmpTan - (tmpNorm.dot(tmpTan) * tmpNorm);
                        Vector3d newB = tmpBin - (tmpNorm.dot(tmpBin) * tmpNorm) - (newT.dot(tmpBin) * newT);
                        ...
                        // should be approx 1.0f
                        if((lenTan <= 0.001f) || (lenBin <= 0.001f))
                        {
                                      ...
                                      Vector3d& binormal) const
        {
                ...
                // let P = v1 - v0
                Vector3d P = v1.pos - v0.pos;

                // let Q = v2 - v0
                Vector3d Q = v2.pos - v0.pos;
                float s1 = v1.s - v0.s;
                float t1 = v1.t - v0.t;
                float s2 = v2.s - v0.s;
                float t2 = v2.t - v0.t;
                ...
                float tmp = 0.0f;
                if(std::fabs(s1 * t2 - s2 * t1) <= 0.0001f)
                        tmp = 1.0f;
                else
